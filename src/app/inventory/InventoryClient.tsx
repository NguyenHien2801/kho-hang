...
export default function InventoryClient() {
  ...
  const V: Record<string, React.CSSProperties> = {
    ...
    wrap:     { maxWidth: 1280, margin: '0 auto', padding: '32px 32px 48px' },
    card:     { background: '#fff', borderRadius: 16, border: '1px solid #e2e8f0', boxShadow: '0 1px 3px rgba(0,0,0,.05)' },
    th:       { textAlign: 'left', padding: '11px 16px', fontSize: 11, fontWeight: 700, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.07em', background: '#f8fafc', borderBottom: '1px solid #e2e8f0', whiteSpace: 'nowrap' },
    td:       { padding: '14px 16px', fontSize: 14, verticalAlign: 'middle' },
    input:    { width: '100%', padding: '10px 13px', borderRadius: 10, border: '1px solid #e2e8f0', background: '#f8fafc', fontSize: 14, color: '#1e293b', outline: 'none', boxSizing: 'border-box', fontFamily: "'Be Vietnam Pro', sans-serif", transition: 'border-color .15s, box-shadow .15s, background .15s' },
    label:    { display: 'block', fontSize: 11, fontWeight: 700, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 7 },
    btnPri:   { display: 'inline-flex', alignItems: 'center', gap: 6, padding: '9px 18px', borderRadius: 10, border: 'none', background: 'linear-gradient(135deg,#6366f1,#4f46e5)', color: '#fff', fontSize: 14, fontWeight: 700, cursor: 'pointer', boxShadow: '0 2px 8px rgba(99,102,241,.3)', fontFamily: "'Be Vietnam Pro', sans-serif" },
    btnGhost: { display: 'inline-flex', alignItems: 'center', gap: 6, padding: '9px 16px', borderRadius: 10, border: '1px solid #e2e8f0', background: '#fff', color: '#475569', fontSize: 14, fontWeight: 600, cursor: 'pointer', fontFamily: "'Be Vietnam Pro', sans-serif" },
  }
    ...
    <div style={V.page}>
      ...
      <div style={V.wrap}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 28 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            ...
            <div>
              <h1 style={{ fontSize: 20, fontWeight: 800, color: '#0f172a', letterSpacing: '-0.025em', margin: 0, lineHeight: 1.2 }}>Quản lý kho hàng</h1>
              <p style={{ fontSize: 13, color: '#94a3b8', margin: '3px 0 0' }}>{products.length} mặt hàng đang theo dõi</p>
            </div>
          </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 16, marginBottom: 24 }}>
          {[
            ...
            { icon: '🚨', label: 'Nhập hàng ngay',  value: criticalCount,   unit: 'sản phẩm', desc: 'Hết trong ≤ 3 ngày',  bg: '#fff1f2', border: '#fecdd3', num: '#be123c', sub: '#fca5a5' },
          ].map((k, i) => (
            <div key={i} style={{ background: k.bg, border: `1px solid ${k.border}`, borderRadius: 16, padding: '20px 22px', boxShadow: '0 1px 4px rgba(0,0,0,.04)' }}>
              <div style={{ fontSize: 28, marginBottom: 10 }}>{k.icon}</div>
              <div style={{ fontSize: 32, fontWeight: 800, color: k.num, letterSpacing: '-0.03em', lineHeight: 1 }}>
                {loading ? '—' : k.value.toLocaleString()}
                <span style={{ fontSize: 13, fontWeight: 600, color: k.num, opacity: .65, marginLeft: 5 }}>{k.unit}</span>
              </div>
              <div style={{ fontSize: 14, fontWeight: 700, color: '#334155', marginTop: 7 }}>{k.label}</div>
              <div style={{ fontSize: 12, color: k.sub, marginTop: 3 }}>{k.desc}</div>
            </div>
          ))}
        </div>
        ...
        <div style={{ display: 'flex', gap: 10, marginBottom: 16, flexWrap: 'wrap' }}>
          ...
          <div style={{ position: 'relative', flex: '1 1 260px', minWidth: 200 }}>
            ...
            <input
              ...
              style={{ ...V.input, padding: '11px 14px 11px 42px', borderRadius: 12 }}
            />
            {search && (
          ...
          <select
            ...
            style={{ ...V.input, width: 'auto', minWidth: 155, padding: '11px 34px 11px 14px', borderRadius: 12, cursor: 'pointer', color: filterCat ? '#6366f1' : '#94a3b8', fontWeight: filterCat ? 700 : 400, borderColor: filterCat ? '#6366f1' : '#e2e8f0', background: filterCat ? '#eef2ff' : '#f8fafc' }}
          >
          ...
          <select
            ...
            style={{ ...V.input, width: 'auto', minWidth: 165, padding: '11px 34px 11px 14px', borderRadius: 12, cursor: 'pointer', color: filterStatus ? '#6366f1' : '#94a3b8', fontWeight: filterStatus ? 700 : 400, borderColor: filterStatus ? '#6366f1' : '#e2e8f0', background: filterStatus ? '#eef2ff' : '#f8fafc' }}
          >
          ...
          {(filterCat || filterStatus) && (
            <button
              ...
              style={{ ...V.btnGhost, padding: '11px 14px', borderRadius: 12, color: '#ef4444', borderColor: '#fecdd3', background: '#fff1f2', whiteSpace: 'nowrap' }}
            >
        ...
        {/* ── TABLE ── */}
        <div style={{ ...V.card, overflow: 'hidden', marginBottom: 16 }}>
                ...
                {loading ? (
                  [...Array(6)].map((_, i) => (
                    <tr key={i} style={{ borderBottom: '1px solid #f1f5f9' }}>
                      {[...Array(9)].map((_, j) => (
                        <td key={j} style={V.td}>
                          <div style={{ height: 12, borderRadius: 6, background: '#f1f5f9', width: j === 0 ? '72%' : '52%' }} />
                        </td>
                      ))}
                    </tr>
                  ))
                  ...
                  <tr>
                    <td colSpan={9} style={{ textAlign: 'center', padding: '72px 0', color: '#94a3b8' }}>
                      ...
                      <div style={{ fontSize: 14, fontWeight: 700 }}>Không tìm thấy sản phẩm</div>
                      <div style={{ fontSize: 13, marginTop: 5 }}>Thử thay đổi từ khoá tìm kiếm hoặc bộ lọc</div>
                    </td>
                  </tr>
                ) : filtered.map((p, idx) => {
                  const st = statusOf(p)
                  return (
                    <tr key={p.id} className="inv-tr" style={{ borderBottom: idx < filtered.length - 1 ? '1px solid #f8fafc' : 'none' }}>
                      ...
                      <td style={V.td}>
                        ...
                        <div style={{ fontSize: 12, color: '#cbd5e1', marginTop: 3, fontWeight: 500 }}>{p.unit}</div>
                      </td>

                      {/* Mã vạch */}
                      <td style={V.td}>
                        <span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 11, color: '#64748b', background: '#f8fafc', padding: '3px 8px', borderRadius: 6, border: '1px solid #f1f5f9', letterSpacing: '.02em' }}>
                          {p.barcode}
                        </span>
                      </td>
                      ...
                      {/* Danh mục */}
                      <td style={V.td}>
                        <span style={{ fontSize: 12, fontWeight: 700, color: '#6366f1', background: '#eef2ff', padding: '4px 10px', borderRadius: 20, whiteSpace: 'nowrap' }}>
                          {p.category}
                        </span>
                      </td>
                      ...
                      <td style={V.td}>
                        ...
                        <span style={{ fontSize: 12, color: '#94a3b8', marginLeft: 4, fontWeight: 500 }}>{p.unit}</span>
                      ...
                      {/* Tối thiểu */}
                      <td style={{ ...V.td, color: '#64748b', fontWeight: 600 }}>{p.min_stock}</td>
                      ...
                      {/* Trạng thái — ô vuông màu */}
                      <td style={V.td}>
                        <div style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: 38, height: 38, borderRadius: 10, background: st.bg, border: `1px solid ${st.border}` }}>
                          <span style={{ width: 20, height: 20, borderRadius: 5, background: st.sqColor, display: 'inline-block' }} />
                        </div>
                      </td>
                      ...
                      <td style={V.td}>
                        <div style={{ display: 'flex', gap: 5 }}>
                          {([
                            { fn: () => openAdjust(p),    icon: <PackagePlus size={18} />, title: 'Nhập/Xuất kho', bg: '#f0fdf4', hbg: '#dcfce7', c: '#15803d' },
                            { fn: () => openEdit(p),      icon: <Pencil size={18} />,      title: 'Chỉnh sửa',    bg: '#eff6ff', hbg: '#dbeafe', c: '#1d4ed8' },
                            { fn: () => deleteProduct(p), icon: <Trash2 size={18} />,      title: 'Xoá',          bg: '#fff1f2', hbg: '#fee2e2', c: '#be123c' },
                          ] as const).map((btn, bi) => (
                            <button key={bi} onClick={btn.fn} title={btn.title}
                              style={{ width: 38, height: 38, borderRadius: 10, border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', background: btn.bg, color: btn.c, transition: 'background .12s, transform .1s' }}
          ...
          {/* ── TABLE FOOTER ── */}
          <div style={{ padding: '14px 20px', borderTop: '1px solid #f1f5f9', background: '#fafafa', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
            {/* Legend */}
        ...
        {/* ── INSIGHT BAR ── */}
        <div style={{ padding: '12px 18px', borderRadius: 12, background: '#f0fdf4', border: '1px solid #bbf7d0', fontSize: 13, color: '#15803d', display: 'flex', alignItems: 'flex-start', gap: 8, lineHeight: 1.6 }}>
          💡 <span><strong>Mẹo:</strong> Thanh màu dưới số tồn kho biểu thị tỷ lệ so với ngưỡng tối thiểu × 2. Ô màu đỏ = hành động ngay hôm nay, vàng = lên kế hoạch trong tuần này.</span>
        </div>
      </div>
      ...
      {(modal === 'add' || modal === 'edit') && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,.55)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50, padding: 16, backdropFilter: 'blur(6px)', animation: 'fadeIn .15s ease' }}>
          <div style={{ background: '#fff', borderRadius: 24, boxShadow: '0 32px 80px rgba(0,0,0,.18)', width: '100%', maxWidth: 520, padding: 32, animation: 'slideUp .22s cubic-bezier(.34,1.4,.64,1)' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 24 }}>
              <div>
                ...
                <p style={{ fontSize: 13, color: '#94a3b8', marginTop: 4, marginBottom: 0 }}>Điền đầy đủ thông tin sản phẩm</p>
              </div>
            ...
            {form.cost_price > 0 && form.sell_price > 0 && (
              <div style={{ marginTop: 14, padding: '10px 14px', borderRadius: 10, background: '#f0fdf4', border: '1px solid #bbf7d0', fontSize: 13, color: '#15803d', display: 'flex', alignItems: 'center', gap: 6 }}>
      ...
      {modal === 'adjust' && selected && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,.55)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50, padding: 16, backdropFilter: 'blur(6px)', animation: 'fadeIn .15s ease' }}>
          <div style={{ background: '#fff', borderRadius: 24, boxShadow: '0 32px 80px rgba(0,0,0,.18)', width: '100%', maxWidth: 400, padding: 32, animation: 'slideUp .22s cubic-bezier(.34,1.4,.64,1)' }}>

            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 20 }}>
              <div>
                <h2 style={{ fontSize: 18, fontWeight: 800, color: '#0f172a', letterSpacing: '-0.025em', margin: 0 }}>Điều chỉnh kho</h2>
                <p style={{ fontSize: 13, color: '#94a3b8', marginTop: 4, marginBottom: 0 }}>{selected.name}</p>
              </div>
            ...
            <div style={{ background: '#f8fafc', borderRadius: 12, padding: '14px 18px', marginBottom: 20, border: '1px solid #f1f5f9', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <span style={{ fontSize: 13, color: '#64748b', fontWeight: 600 }}>Tồn hiện tại</span>
              <span style={{ fontSize: 24, fontWeight: 800, color: '#0f172a' }}>
                {selected.stock} <span style={{ fontSize: 13, fontWeight: 600, color: '#94a3b8' }}>{selected.unit}</span>
              </span>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8 }}>
                  {([
                    ['in',     'Nhập kho',   '#15803d', '#f0fdf4', '#bbf7d0'],
                    ['out',    'Xuất kho',   '#be123c', '#fff1f2', '#fecdd3'],
                    ['adjust', 'Điều chỉnh', '#4f46e5', '#eef2ff', '#c7d2fe'],
                  ] as const).map(([t, label, c, bg]) => (
                    <button key={t} onClick={() => setAdjType(t as 'in' | 'out' | 'adjust')}
                      style={{ padding: '10px 4px', borderRadius: 10, border: `1.5px solid ${adjType === t ? c : '#e2e8f0'}`, background: adjType === t ? bg : '#fff', color: adjType === t ? c : '#64748b', fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: "'Be Vietnam Pro',sans-serif", transition: 'all .15s' }}>
                      {label}
                    </button>
                  ))}
                </div>
              ...
              <div style={{ borderRadius: 12, border: '1.5px dashed #e2e8f0', padding: '12px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#fafafa' }}>
                <span style={{ fontSize: 13, color: '#64748b', fontWeight: 600 }}>Tồn sau điều chỉnh</span>
                <span style={{ fontSize: 22, fontWeight: 800, color: '#0f172a' }}>
                  {Math.max(0, selected.stock + (adjType === 'out' ? -adjQty : adjQty))}
                  <span style={{ fontSize: 13, fontWeight: 600, color: '#94a3b8', marginLeft: 4 }}>{selected.unit}</span>
                </span>
              </div>
            </div>
